        ...
        */
        _setChildNodes: function(newVChildNodes) {
            ...
            var instance = this,
                vChildNodes = instance.vChildNodes,
                domNode = instance.domNode,
                i, oldChild, newChild, newLength, len, newChildDomNode, childDomNode, nodeswitch;
...
/*jshint boss:false */
                        case 1: // oldNodeType==Element, newNodeType==Element
                            if ((oldChild.tag!==newChild.tag) || ((oldChild.tag==='SCRIPT') && (oldChild.text!==newChild.text))) {
                                ...
                                newChildDomNode = DOCUMENT.createElement(newChild.tag);
                                newChild._setAttrs(newChild.attrs);
                                domNode._replaceChild(newChildDomNode, childDomNode);
                                newChild._setChildNodes(newChild.vChildNodes);
                                newChild.vParent = instance;
                                newChild.id  && (nodeids[newChild.id]=newChildDomNode);
                        ...
                        case 2: // oldNodeType==Element, newNodeType==TextNode
                            newChildDomNode = DOCUMENT.createTextNode(newChild.text);
                            domNode.replaceChild(newChildDomNode, childDomNode);
                            newChild.vParent = instance;
                            oldChild._replaceAtParent(newChild);
                            break;
                        case 3: // oldNodeType==Element, newNodeType==Comment
                            newChildDomNode = DOCUMENT.createComment(newChild.text);
                            domNode.replaceChild(newChildDomNode, childDomNode);
                            newChild.vParent = instance;
                            oldChild._replaceAtParent(newChild);
                            break;
            ...
            for (i = len; i < newLength; i++) {
                ...
                switch (newChild.nodeType) {
                    case 1: // Element
                        ...
                        newChild._setChildNodes(newChild.vChildNodes);
                        newChild.domNode = newChildDomNode;
                        break;
                    case 3: // TextNode
                        newChildDomNode = DOCUMENT.createTextNode(newChild.text);
                        domNode.appendChild(newChildDomNode);
