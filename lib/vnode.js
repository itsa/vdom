    ...
    vNodeProto = {
        ...
        */
        _remove: function() {
            ...
            // explicitely set instance.domNode to null in order to prevent problems with the GC
        ...
        */
        _setAttrs: function(newAttrs) {
            ...
            id ? (nodeids[id]=instance.domNode) : (delete nodeids[id]);
...
/*jshint boss:false */
                        case 1: // oldNodeType==Element, newNodeType==Element
                            if ((oldChild.tag!==newChild.tag) || ((oldChild.tag==='SCRIPT') && (oldChild.text!==newChild.text))) {
                                ...
                                newChildExtNode = DOCUMENT.createElement(newChild.tag);
                                ...
                                domNode._replaceChild(newChildExtNode, childExtNode);
                                ...
                                oldChild.domNode._updateLifeLists(true);
                            }
                            else {
                                ...
                                newChild._setChildNodes(newChild.vChildNodes);
                                newChild.attrs['class'] && newChild.domNode._updateLifeClassLists();
                                oldChild.attrs['class'] && oldChild.domNode._updateLifeClassLists(true);
                                newChild.attrs.name && newChild.domNode._updateLifeNamesList();
                                oldChild.attrs.name && oldChild.domNode._updateLifeNamesList(true);
                            }
                            break;
                        case 2: // oldNodeType==Element, newNodeType==TextNode
                            newChildExtNode = DOCUMENT.createTextNode(newChild.text);
                            domNode._replaceChild(newChildExtNode, childExtNode);
                            newChild.vParent = instance;
                            oldChild._replaceAtParent(newChild);
                            oldChild.domNode._updateLifeLists(true);
                        ...
                        case 3: // oldNodeType==Element, newNodeType==Comment
                            newChildExtNode = DOCUMENT.createComment(newChild.text);
                            domNode._replaceChild(newChildExtNode, childExtNode);
                            newChild.vParent = instance;
                            oldChild._replaceAtParent(newChild);
                            oldChild.domNode._updateLifeLists(true);
            ...
            for (i = len; i < newLength; i++) {
                ...
                switch (newChild.nodeType) {
                    case 1: // Element
                        ...
                        domNode._appendChild(newChildExtNode);
                        newChild._setChildNodes(newChild.vChildNodes);
                        newChild.domNode = newChildExtNode;
                        newChildExtNode._updateLifeLists();
                        break;
                    case 3: // TextNode
                        newChildExtNode = DOCUMENT.createTextNode(newChild.text);
            ...
            set: function(v) {
                ...
                if (vnode.nodeType===1) {
                    if (vnode.tag!==instance.tag) {
                        ...
                        instance.vParent.domNode.replaceChild(newElement);
                        instance.domNode = newElement;
                    }
        ...
        textContent: {
            ...
            set: function(v) {
                var instance = this,
                    ...
                    vChildNodes = instance.vChildNodes,
                    len, i, vChildNode;
                domNode.textContent = v;
                // set .text AFTER the dom-node is updated --> the content might be escaped!
                instance.text = domNode.textContent;
                if (vChildNodes) {
                    len = vChildNodes.length;
                    for (i=0; i<len; i++) {
                        vChildNode = vChildNodes[i];
